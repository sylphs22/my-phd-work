...
//	  err_fin = 0 ;
		...
		///PQ= PQ+sum/(sqrtf(norm1*norm2)) ;
				 ...
				 for ( int e = 0 ; e < f_dec ; e++ )
			{
			//	printf("\n New_R[%d][%d] Q[%d][%d] %f %f",i,j,e,b,New_R[i][j],Q[e][b]);
			//	_getch();
		A[e]=A[e]+New_R[i][j]*(sqrtf(norm1*norm2)*Q[e][b]-sum*sqrtf(norm2/norm1)*P[i][e])/(norm1*norm2);
		//A[e]=A[e]+(((Q[e][b]*sum*norm1*norm2)-(2*sum*sum*(norm2)*P[i][e]))/(norm1*norm2*norm1*norm2));
		//B[e]=B[e]+(((Q[e][b]*sqrtf(norm1*norm2)*New_R[i][j])-(New_R[i][j]*sum*sqrtf(norm2/norm1)*P[i][e]))/(norm1*norm2));
			 }
		...
		//printf("\n  A[%d]  %f",e,A[e]);
		//_getch();
				P[i][e]=P[i][e]+0.41*(A[e]);//-0.001*P[i][e]);					
}//j
}//i
for(int j =0;j<Ry;j++)
	{
       ...
       for(int k =0;k<Rx;k++)
	{
			 ...
			 for ( int e = 0 ; e < f_dec ; e++ )
				{
				 ...
				 sum = sum + P[k][e]*Q[e][b];
										...
										//			printf ("\n Q[%d][%d] %f",e,j,Q[e][j]);
										//			_getch();
                 norm1=norm1+ P[k][e]*P[k][e];//;C[i]*C[i]
													//  printf("\norm1 %f",norm1);
													//_getch();
			     norm2=norm2+(Q[e][b]*Q[e][b]);
										// printf("\n norm2[c] %f",norm2[c]);
		...
		//	err_mid=(((P[i][e]*sum*norm1*norm2)-(2*sum*sum*(norm1)*Q[e][b]))/(norm1*norm2*norm1*norm2));
			C[e]=C[e]+New_R[k][b]*(sqrtf(norm1*norm2)*P[k][e]-sum*sqrtf(norm1/norm2)*Q[e][b])/(norm1*norm2);
		//	err_fin=(((P[i][e]*sqrtf(norm1*norm2)*New_R[i][j])-(New_R[i][j]*sum*sqrtf(norm1/norm2)*Q[e][b]))/(norm1*norm2));
...
for(int k =0;k<Rx;k++)
	{    b=0;b=S[k][j];
			if(New_R[k][j]==0) 
				...
				//	if(New_R[k][j]==S[k][b]){continue;}
	   for ( int e = 0 ; e < f_dec ; e++ )
		{
			//printf("\n PQPQ  %f ",(C[e]));
			//_getch();
			Q[e][b]=Q[e][b]+0.41*((C[e]));//-0.001*Q[e][b]);
	   }
		}
}
/*  err_prev = 0 ;
